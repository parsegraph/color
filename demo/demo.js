...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>color</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>color <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-color.js'>parsegraph-color.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See color build information at http://localhost:${port}`);
});
